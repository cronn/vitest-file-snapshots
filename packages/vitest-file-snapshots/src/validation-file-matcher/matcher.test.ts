...
import { registerValidationFileMatcher } from "./matcher";

describe("with default validation file options", () => {
  beforeEach(() => registerValidationFileMatcher());
...
describe("with custom validation file options", () => {
  ...
  test("applies custom directories", () => {
    expect("value").toMatchValidationFile();
  });
});
